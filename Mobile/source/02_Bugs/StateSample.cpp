...
void GEStateSample::internalInit()
{
   ...
   cRender->createLabel(&cTextEscaped, 0, TopRight, Vector2(0.1f, 0.1f), "Escaped: 0");
   cTextEscaped->setPosition(0.9f, 1.3f);
...
void GEStateSample::generateBug()
{
   ...
   switch(iType) 
   {
      ...
      case 0:         
         ...
         sBug.Position.Y = BOUNDS_TOP + fMargin;
         sBug.Destiny.X = (float)(-100 + rand() % 200) * 0.01f; 
         sBug.Destiny.Y = BOUNDS_BOTTOM - fMargin;
         break;
      
      // down to up
      case 1:
         sBug.Position.X = (float)(-100 + rand() % 200) * 0.01f;
         sBug.Position.Y = BOUNDS_BOTTOM - fMargin;
         sBug.Destiny.X = (float)(-100 + rand() % 200) * 0.01f; 
         sBug.Destiny.Y = BOUNDS_TOP + fMargin;
         break;
         
      // left to right
      case 2:
         sBug.Position.X = BOUNDS_LEFT - fMargin;
         sBug.Position.Y = (float)(-150 + rand() % 300) * 0.01f;
         sBug.Destiny.X = BOUNDS_RIGHT + fMargin;
         sBug.Destiny.Y = (float)(-150 + rand() % 300) * 0.01f; 
         break;
         
      // right to left
      case 3:
         sBug.Position.X = BOUNDS_RIGHT + fMargin;
         sBug.Position.Y = (float)(-150 + rand() % 300) * 0.01f;
         sBug.Destiny.X = BOUNDS_LEFT - fMargin;
         sBug.Destiny.Y = (float)(-150 + rand() % 300) * 0.01f;
         break;
   }
